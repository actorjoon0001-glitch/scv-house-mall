...
// 발송: 솔라피(Solapi) REST API. 필요한 환경변수(Netlify 대시보드에 설정):
//   SOLAPI_API_KEY, SOLAPI_API_SECRET  — 솔라피 콘솔 발급
//   SMS_FROM                            — 사전 등록된 발신번호 (예: 01012345678)
...
exports.handler = async (event) => {
  ...
  if (!SRK || !SOL_KEY || !SOL_SEC || !SMS_FROM) return json(503, { error: "not_configured" });
  ...
  const smsRes = await fetch("https://api.solapi.com/messages/v4/send", {
    ...
    body: JSON.stringify({
      message: { to: phone, from: SMS_FROM, text: `[메타하우스] 인증번호 [${code}]를 입력해주세요. (5분 이내)` },
    }),
  });
